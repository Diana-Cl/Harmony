...
از هر کد وورکر که تمایل دارید برای ساخت یک کانفیگ `Vless` استفاده کنید. برای ما دو عنصر کلیدی در این پروسه فقط `UUID` و `hostname` کانفیگ شما هستند.

### 1. ایجاد کانفیگ Vless

من برای مثال از این وورکر ساده و قدیمی برای ساخت کانفیگ VLESS استفاده کردم، کد وورکر تو فایل worker.js هست باید کپی کنید. اگر کد مبهم نشده خواستید هم فایل `index.js` رو کپی و استفاده کنید

<a href="https://github.com/NiREvil/zizifn?utm_source=github&utm_campaign=harmony-readme"><img src="https://github.com/amnezia-vpn/amnezia-client/blob/dev/metadata/img-readme/download-website.svg" width="150" style="max-width: 100%; margin-right: 10px"></a>

<br/> 

> [!WARNING]
> 
> اگر کانفیگ ندارید و تاکنون وورکر نساخته‌اید، توضیحات پیرامون ساخت کانفیگ رو بخوانید. در غیر این صورت، این قسمت رو نادیده گرفته و آموزش رو ادامه بدید.
...
همونطور که بالاتر گفتم من از کد وورکر [zizifn][1] برای ساخت وورکر و ایجاد کانفیگ استفاده کردم، شما می‌تونید از BPB یا هر کد دیگری به دلخواه استفاده کنید. بعد از حپی کردن کد کافیه وارد اکانت کلادفلر خودتون بشید و بعد:
- بالای صفحه روی آیکون + کلیک کنید
- روی workers بزنید
- گزینه start with hello world رو انتخاب کنید
- یه اسم انتخاب کنید براش و Deploy رو بزنید
- روی گزینه Edit code بزنید تا ویرایشگر باز بشه
- 
...
> این روش توصیه می‌شود چون روش بالا به دلیل سختگیری‌های اخیر کلادفلر ممکن است هنگام ساخت وورکر با مشکل روبرو بشه و خطای 1101 نشون بده .
برای ساخت کانفیگ از طریق Pages، کافیه این مخزن رو فورک کرده و از داخل اکانت کلادفلر خود یک پیج جدید ایجاد کنید. حتماً از این وب‌سایت [UUID Generator] یک آی‌دی جدید گرفته و موقع ساخت پیج در قسمت `environment variables` یک متغیر جدید ایجاد کرده و نام آن را `UUID` و مقدارش را برابر با آی‌دی که از سایت گرفتید قرار دهید.
...
در این مرحله در صفحه‌ای که باز می‌شود، می‌توانید کانفیگ Vless آماده خود را کپی و داخل کلاینت دلخواه وارد و استفاده کنید. ولی ما اینجا فقط Hostname و UUID را کپی می‌کنیم و میریم برای ادامه آموزش ساخت لینک‌ ساب‌اسکزیپشن از نوع هارمونی.
...
- **2.1 دانلود کد**
  ...
  ابتدا کد هارمونی رو کپی یا دانلود کنید، یا اگه از مخزن فورک زدید روی همون فورک خودتون داخل گیتهاب ویرایش کنید. [^2]
